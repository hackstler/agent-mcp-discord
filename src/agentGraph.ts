...
.addNode('fetchMessages', async (state) => {
  console.log('🔎 Searching channel by ID:', state.channelId);

  try {
    const rawChannel = await client.channels.fetch(state.channelId);
    if (!rawChannel) throw new Error('The channel does not exist or is not accessible');

    if (rawChannel.type !== 0) throw new Error('The channel is not a text channel');
    ...
    console.error('❌ Error accessing the channel:', err.message);
